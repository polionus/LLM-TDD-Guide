...
**Hello ritual checklist:**
1. Read `workflow/workbench.md`
2. Read `workflow/log.md`
3. Summarize current state and next action to the user
4. Wait for confirmation before doing any work

**Every session ends with a goodbye ritual** — triggered by saying *"Goodbye"*. The LLM works through the checklist below in order and confirms completion before closing.

**Goodbye ritual checklist:**
1. Update `workflow/workbench.md` — current focus, done list, next action
2. Append to `workflow/log.md` — `session:closed` entry
3. Propose mindmap updates for any components that changed status; write after confirmation
4. GC pass — scan all source files for dead code and unused imports; fix trivial issues, log non-trivial ones to `workflow/debt.md`
5. Add active learning prompts to `workflow/techniques.md` for each component completed this session
